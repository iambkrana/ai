        ...
        if ($cmp_id != '' && $feedbacktype_id != '') {
            ...
            if (count((array)$lang_key) > 0) {
                foreach ($lang_key as $lk) {
                    $result = $translate->translate($subtype, ['target' => $lk]);
            ...
            if (count((array)$final_txt) > 0) {
                $querystr = "SELECT description from feedback_subtype where LOWER(REPLACE(description, ' ', ''))   IN ('" . implode("','", $final_txt) . "')  ";
                if ($cmp_id != '') {
                ...
                if (count((array)$data) > 0) {
                    echo $msg = "Feedback Subtype already exists..!!";
                }
            }
            // Changes by Bhautik Rana - Language module changes-28-02-2024
        }
