        ...
        if ($cmp_id != '') {
            ...
            if (count((array)$lang_key) > 0) {
                foreach ($lang_key as $lk) {
                    $result = $translate->translate($store_id, ['target' => $lk]);
            ...
            if (count((array)$final_txt) > 0) {
                $querystr = "SELECT id from store_mst where LOWER(REPLACE(store_name, ' ', '')) IN ('" . implode("','", $final_txt) . "')  "
                    . " and company_id=" . $cmp_id;
                if ($store_Eid != '') {
                    $querystr .= " and id!=" . $store_Eid;
                }
    $result = $this->db->query($querystr);
                $data = $result->row();
                if (count((array)$data) > 0) {
                    echo $msg = "Store Name already exists....";
                }
            }
            // Changes by Bhautik Rana - Language module changes-28-02-2024
        }
