        ...
        if ($cmp_id != '') {
            ...
            if (count((array)$lang_key) > 0) {
                foreach ($lang_key as $lk) {
                    $result = $translate->translate($type, ['target' => $lk]);
            ...
            if (count((array)$final_txt) > 0) {
                $querystr = "Select description from feedback_type where LOWER(REPLACE(description, ' ', ''))  IN ('" . implode("','", $final_txt) . "') ";
                if ($cmp_id != '') {
                ...
                if (count((array)$data) > 0) {
                    echo $msg = "Feedback type already exists....";
                }
            }
            // Changes by Bhautik Rana - Language module changes-28-02-2024
        }
