...
class Parameterlabel extends MY_Controller {
    ...
    public function DatatableRefresh() {
        ...
        $DTRenderArray = [];
        $this->db->select("p.*,pm.description as parameter_name");
        $this->db->from('parameter_label_mst as p');
        $this->db->join('parameter_mst as pm', 'p.parameter_id = pm.id', 'left');
        ...
        if (!$acces_management->allow_add) {
            ...
            if ($SuccessFlag) {
                ...
                $data = array(
                    ...
                    //'description'           => $this->security->xss_clean($this->clean($this->input->post('description'))), -- shital LM 07-03-2024
    ...
    public function Check_subparameter() {
        ...
        if ($parameterlabel != '') {
                ...
                /* if (!preg_match("/^[a-zA-Z0-9-_ ]+$/", $parameterlabel)) {
                    echo "Please enter a valid description";
                } */
                //$this->parameterlabel_model->Check_subparameter_exist($parameterlabel,$subparameter_id);
            ...
            $translate = new TranslateClient(['key' => 'AIzaSyBFbgPZh0xg8rcH_vkWEXtWufNxwVvU750']);
                ...
                $result_PK = $translate->translate($parameterlabel, ['target' => 'en']);
            ...
            if (!preg_match("/^[a-zA-Z0-9-_ ]+$/", $final_text)) {
                ...
                if (count((array)$lang_key) > 0) {
                    foreach ($lang_key as $lk) {
                        $result = $translate->translate($parameterlabel, ['target' => $lk]);
                ...
                if (count((array)$final_txt) > 0) {
                    $query = "select description from parameter_label_mst where LOWER(REPLACE(description, ' ', '')) IN ('" . implode("','", $final_txt) . "') ";
                    if($subparameter_id!=''){
                        $query.=" and id!=".$subparameter_id;
                    }
                    
                    $result = $this->db->query($query);
                    $data = $result->row();
                    if (count((array)$data) > 0) {
                        echo $msg = "Parameter Label already exists!!!";
                    }
                } // Changes by  Shital Patel - Language module changes-07-03-2024
