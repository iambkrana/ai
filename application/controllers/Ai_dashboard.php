...
class Ai_dashboard extends MY_Controller
{
    ...
    public function fetch_statistics()
    {
        ...
        $_statistics_start_date = $this->security->xss_clean($this->input->post('statistics_start_date', true));
        $_statistics_end_date = $this->security->xss_clean($this->input->post('statistics_end_date', true));
        $box_i_statistics = 0;
        $box_ii_statistics = 0;
        ...
        $Whercluse = " (date(start_dttm) BETWEEN '" . $_statistics_start_date . "' AND '" . $_statistics_end_date . "' 
        ...
        $this->db->select('count(*) as total')->from('assessment_mst AS am');
        $this->db->where('am.STATUS', 1);
        $this->db->where('am.company_id', $_company_id);
        if ($this->mw_session['role'] == 4) {
            $this->db->where('am.division_id', $division_id);
        }
        $this->db->group_start()
            ->where('date(start_dttm) >=', $_statistics_start_date)
            ->where('date(start_dttm) <=', $_statistics_end_date)
            ->or_group_start()
            ->where('date(end_dttm) >=', $_statistics_start_date)
            ->where('date(end_dttm) <=', $_statistics_end_date)
            ->group_end()
            ->or_group_start()
            ->where('date(start_dttm) <=', $_statistics_start_date)
            ->where('date(end_dttm) >=', $_statistics_end_date)
            ->group_end()
            ->group_end();
        // $this->db->where($Whercluse);
        $box_i_result = $this->db->get()->row();
        ...
        $where2 = " date(sent_at) BETWEEN '" . $_statistics_start_date . "' AND '" . $_statistics_end_date . "'";
        $this->db->select('count(*) as total')->from('trainee_report_schedule');
        $this->db->where('is_sent', 1);
        $this->db->where('company_id', $_company_id);
        $this->db->where('date(sent_at) >=', $_statistics_start_date);
        $this->db->where('date(sent_at) <=', $_statistics_end_date);
        // $this->db->where($where2);
        $box_vi_result = $this->db->get()->row();
        ...
        $where3 = " (date(am.start_dttm) BETWEEN '" . $_statistics_start_date . "' AND '" . $_statistics_end_date . "'
        ...
        $this->db->distinct('total');
        $this->db->select('count(aau.user_id) as total');
        $this->db->from('assessment_allow_users as aau');
        $this->db->join('assessment_mst as am', 'aau.assessment_id = am.id', 'left');
        $this->db->join('device_users as du', 'aau.user_id=du.user_id', 'left');
        $this->db->where('am.status', 1);
        $this->db->where('am.company_id', $_company_id);
        ...
        $this->db->group_start()
            ->where('date(am.start_dttm) >=', $_statistics_start_date)
            ->where('date(am.start_dttm) <=', $_statistics_end_date)
            ->or_group_start()
            ->where('date(am.end_dttm) >=', $_statistics_start_date)
            ->where('date(am.end_dttm) <=', $_statistics_end_date)
            ->group_end()
            ->or_group_start()
            ->where('date(am.start_dttm) <=', $_statistics_start_date)
            ->where('date(am.end_dttm) >=', $_statistics_end_date)
        ...
        $this->db->select('assessment_id,aa.user_id,count(aa.user_id) as played,sum(IF(is_completed=1,1,0)) as completed ');
        $this->db->from('assessment_attempts as aa');
        $this->db->join('assessment_mst as am', 'aa.assessment_id = am.id', 'left');
        $this->db->join('device_users as du', 'du.user_id = aa.user_id', 'left');
        $this->db->where('am.status', 1);
        $this->db->where('am.company_id', $_company_id);
        $this->db->where('du.istester', 0);
        if ($this->mw_session['role'] == 4) {
        ...
        $this->db->group_start()
            ...
            ->group_end()
            ->group_end();
        $this->db->where('DATE_FORMAT(aa.complete_dttm,"%Y-%m-%d") >=', $_statistics_start_date);
        $this->db->where('DATE_FORMAT(aa.complete_dttm,"%Y-%m-%d") <=', $_statistics_end_date);
        $this->db->group_by(array("assessment_id", "aa.user_id"));
        $sub_query = $this->db->get_compiled_select();

        $this->db->select('sum(played) as played,sum(completed) as completed');
        $this->db->from("($sub_query) as main");
        $box_iii_result = $this->db->get()->row();
        ...
        $this->db->distinct('ar.company_id');
        $this->db->select('ar.company_id,ar.assessment_id,ar.user_id,ar.trans_id,ar.question_id');
        ...
        $this->db->where('am.status', 1);
        $this->db->where('aa.ftpto_vimeo_uploaded', 1);
        ...
        $this->db->group_start()
            ...
            ->group_end()
            ->group_end();
        $this->db->where('DATE_FORMAT(ar.addeddate,"%Y-%m-%d") >=', $_statistics_start_date);
        $this->db->where('DATE_FORMAT(ar.addeddate,"%Y-%m-%d") <=', $_statistics_end_date);
        $sub_query = $this->db->get_compiled_select();

        $this->db->select('count(*) as total');
        $this->db->from("($sub_query) as main");
        $box_iv_result = $this->db->get()->row();
        ...
        $where4 = " DATE_FORMAT(ai.task_status_dttm,'%Y-%m-%d') BETWEEN '" . $_statistics_start_date . "' AND '" . $_statistics_end_date . "' ";
        $this->db->select('count(*) as total');
        $this->db->from('ai_schedule as ai');
        $this->db->join('device_users as du', 'ai.user_id=du.user_id', 'left');
        $this->db->join('assessment_mst as am', 'ai.assessment_id = am.id', 'left');
        $this->db->where('am.status', 1);
        $this->db->where('ai.company_id', $_company_id);
        $this->db->where('ai.task_status', 1);
        if ($this->mw_session['role'] == 4) {
            $this->db->where('am.division_id', $division_id);
        }
        $this->db->where('du.istester', 0);
        $this->db->where('DATE_FORMAT(ai.task_status_dttm,"%Y-%m-%d") >=', $_statistics_start_date);
        $this->db->where('DATE_FORMAT(ai.task_status_dttm,"%Y-%m-%d") <=', $_statistics_end_date);
        // $this->db->where($where4);
        $box_v_result = $this->db->get()->row();
    ...
    public function fetch_assessment()
    {
        ...
        $DTRenderArray = [];
        $this->db->select("am.id,am.company_id,am.assessment_type,am.assessment,at.description AS assessment_type, art.description as report_type,IF(is_situation=1,'Situation','Question') AS question_type,am.status,DATE_FORMAT(am.start_dttm,'%d-%m-%Y %H:%i') AS start_dttm,DATE_FORMAT(am.end_dttm,'%d-%m-%Y %H:%i') AS end_dttm,IFNULL(ac.show_reports,0) as show_reports,IFNULL(ac.show_dashboard,0) as show_dashboard,IFNULL(ac.show_pwa,0) as show_pwa, IFNULL(ac.show_ranking,0) as show_ranking, count(DISTINCT ar.question_id) as que_mapped");
        $this->db->from('assessment_mst as am');
        $this->db->join('assessment_trainer_result as atr', 'atr.assessment_id = am.id', 'left');
        ...
        $this->db->join('assessment_trans as ar', 'ar.assessment_id = am.id', 'left');
        $this->db->join('ai_cronreports as ac', 'ac.assessment_id=am.id', 'left');
    ...
    public function CandidateDatatableRefresh($assessment_id, $is_send_tab)
    {
        ...
        $DTRenderArray = $this->ai_dashboard_model->get_distinct_participants($company_id, $assessment_id, $division_id);
        $output = array(
            "sEcho" => $this->input->get('sEcho') ? $this->input->get('sEcho') : 0,
            "iTotalRecords" => $DTRenderArray['dtPerPageRecords'],
            "iTotalDisplayRecords" => $DTRenderArray['dtTotalRecords'],
        ...
        $userid_rating_array = array_unique(array_column($user_rating_array, 'user_id'));
    ...
    public function fetch_participants()
    {
        ...
        $_participants_result = $this->ai_dashboard_model->get_distinct_participants($company_id, $asssessment_id, $division_id);
    ...
    public function update_template()
    {
        ...
        $this->form_validation->set_rules('label', 'Lebel', 'trim|required');
        $this->form_validation->set_rules('subject', 'Subject', 'trim|required');
        $this->form_validation->set_rules('message', 'Message', 'required');
        $this->form_validation->set_rules('fromemail', 'From Email', 'trim|required|valid_email');
        if ($this->form_validation->run() == FALSE) {
            $Message = validation_errors();
            $SuccessFlag = 0;
        } else {
            if ($SuccessFlag) {
                //KRISHNA -- VAPT -- Access control bypass
                $from_email = $this->security->xss_clean($this->input->post('fromemail'));
                if (filter_var($from_email, FILTER_VALIDATE_EMAIL)) { //Check for valid email address
                    $data = array(
                        // 'alert_title'   => $this->security->xss_clean($this->input->post('label')),
                        'subject' => $this->security->xss_clean(strip_tags($this->input->post('subject'))),
                    ...
                    $Message = "$from_email is not a valid email address";
                    $SuccessFlag = 0;
                }
            }
        }
                    ...
                    foreach ($ideal_url as $key => $value) {
                        if (!empty($value)) {
                            ...
                            if (empty($check_quetion_id)) {
                                ...
                                if (strpos($value, '>') !== false || strpos($value, '<') !== false) { //KRISHNA --- VAPT - NOT ALLOW HTML OR JAVASCRIPT TAGS
                                    $SuccessFlag = 0;
                                    $Message = 'Invalid ideal URL!';
                                } else {
                                    $data = array(
                                        'assessment_id' => $this->security->xss_clean($this->input->post('assessment_id')),
                                        'question_id' => $key,
                                        'best_video_link' => strip_tags($value),
                                    );
                        ...
                        if (empty($check_update_quetion_id)) {
                            ...
                            if (strpos($value, '>') !== false || strpos($value, '<') !== false) { //KRISHNA --- VAPT - NOT ALLOW HTML OR JAVASCRIPT TAGS
                                ...
                                if (!empty($id_video)) {
                                    $id = $id_video[0]->id;
                                    if (!empty($value)) {
                                        $this->common_model->update('ai_best_ideal_video', 'id', $id, ['best_video_link' => strip_tags($value)]);
                                    } else {
