...
class Assessment_dashboard extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $data['TrainerResult'] = $this->common_model->get_selected_values('company_users', 'userid,CONCAT(first_name, " " ,last_name) as fullname', 'status="1" AND role="2" AND company_id="' . $company_id . '"');
        ...
        $data['division_data'] = $this->assessment_dashboard_model->get_division($division_id);
// echo '<pre>';
// print_r($data['division_data']);exit;
    ...
    public function getdashboardData()
    {
        ...
        $data['Total_Assessment'] = $this->assessment_dashboard_model->get_Total_Assessment($Company_id, $SDate, $EDate, $region_id, $store_id, $report_type, $division_id, $trainer_id);
        $data['Candidate_Count'] = $this->assessment_dashboard_model->get_Candidate_Assessment($Company_id, $question_type, $report_by, $SDate, $EDate, $region_id, $store_id, $report_type, $division_id, $trainer_id);
        $data['Avg_Accuracy'] = $this->assessment_dashboard_model->get_Average_Accuracy($Company_id, $report_by, $SDate, $EDate, $region_id, $store_id, $report_type, $assessment_id1, $division_id, $trainer_id);
        $TopFiveParameter = $this->assessment_dashboard_model->get_top_five_parameter($Company_id, $report_by, $SDate, $EDate, $region_id, $store_id, $report_type, $assessment_id1, $division_id, $trainer_id);
        ...
        if (count((array) $TopFiveParameter) > 0) {
            ...
            $minSet = $this->assessment_dashboard_model->get_bottom_five_parameter($Company_id, $report_by, $existid, $SDate, $EDate, $region_id, $store_id, $report_type, $assessment_id1, $division_id, $trainer_id);
        ...
        $BottomFiveParameter = $this->assessment_dashboard_model->get_bottom_five_parameter($Company_id, $report_by, $top_five_para_id, $SDate, $EDate, $region_id, $store_id, $report_type, $assessment_id1, $division_id, $trainer_id);
        ...
        if (count((array) $session_set) > 0) {
            ...
            if ($report_by == 1) {
                $region_data = $this->assessment_dashboard_model->overall_result_parameter(1, $passrange_from, $passrange_to, $failrange_from, $failrange_to, $SDate, $EDate, '', '', '', $store_id, $report_type, $division_id, $trainer_id);
            } else {
                $region_data = $this->assessment_dashboard_model->overall_result_assessment(1, $passrange_from, $passrange_to, $failrange_from, $failrange_to, $SDate, $EDate, '', '', '', $store_id, $report_type, $division_id, $trainer_id);
            }
    ...
    public function getRegionChart($returnflag = 0)
    {
        ...
        if (count((array) $session_set) > 0) {
            ...
            $failrange_to = $session_set['Fail']['range_to'];
            if ($report_by == 1) {
                $region_data = $this->assessment_dashboard_model->overall_result_parameter(0, $passrange_from, $passrange_to, $failrange_from, $failrange_to, $StartDate, $EndDate, $region_id, $assessment_id, $step, $store_id, $report_type, $division_id, $trainer_id);
            } else {
                $region_data = $this->assessment_dashboard_model->overall_result_assessment(0, $passrange_from, $passrange_to, $failrange_from, $failrange_to, $StartDate, $EndDate, $region_id, $assessment_id, $step, $store_id, $report_type, $division_id, $trainer_id);
            }
            if (isset($region_data['region_data']) and count((array) $region_data['region_data']) > 0) {
                $pass_color_code = $this->session->userdata('Assessment_result_session')['Pass']['range_color'];
                $fail_color_code = $this->session->userdata('Assessment_result_session')['Fail']['range_color'];
    ...
    public function getFilterDashboardData($returnflag = 0)
    {
        ...
        $region_list = array();
        $regionset = $this->assessment_dashboard_model->get_region_result($Company_id, $report_by, $region_id, $StartDate, $EndDate, $assessment_id, $report_type, $supervisor_id, $division_id, $trainer_id);
        $last_avg = array();
        ...
        if (count((array) $regionset) > 0) {
            if ($report_by == 1) {
                $Avgset = $this->assessment_dashboard_model->get_region_average($Company_id, 1, $region_id, $StartDate, $EndDate, $assessment_id, $report_type, $store_id, $division_id, $trainer_id);
                $Avgset2 = $this->assessment_dashboard_model->get_region_average($Company_id, 3, $region_id, $StartDate, $EndDate, $assessment_id, $report_type, $store_id, $division_id, $trainer_id);
            } else {
                $Avgset = $this->assessment_dashboard_model->get_region_average($Company_id, 2, $region_id, $StartDate, $EndDate, $assessment_id, $report_type, $store_id, $division_id, $trainer_id);
                $Avgset2 = $this->assessment_dashboard_model->get_region_average($Company_id, 4, $region_id, $StartDate, $EndDate, $assessment_id, $report_type, $store_id, $division_id, $trainer_id);
            }
    ...
    public function parameter_scoredata()
    {
        ...
        $data['trainer_id'] = $this->input->post('trainer_id', true);
        $data['division_id'] = $this->input->post('division_id', true);
        $start_date = $this->input->post('StartDate', true);
        $end_date = $this->input->post('EndDate', true);
        $SDate = date('Y-m-d', strtotime($start_date));
        $EDate = date('Y-m-d', strtotime($end_date));
        $data['assessment_result'] = $this->assessment_dashboard_model->get_parameter_assessment_score($data['company_id'], $data['parameter_id'], $data['region_id'], $SDate, $EDate, $data['report_type'], $data['store_id'], $data['division_id'], $data['trainer_id']);
    ...
    public function QuestionScoreDatatable()
    {
        ...
        if ($division_id != '') {
            $dtWhere .= " AND asm.division_id =" . $division_id;
        }
        if ($trainer_id != '') {
            $dtWhere .= " AND asm.trainer_id IN ($trainer_id) ";
        }
        ...
        if ($SuccessFlag) {
            foreach ($range_id as $key1 => $range_val) {
                $Threshold_array[$range_val] = array(
                    ...
                    'range_to' => $this->input->post('range_to')[$key1],
                    'range_color' => $this->input->post('range_color')[$key1],
                );
            }
            foreach ($result_status as $key2 => $result_val) {
                $result_array[$result_val] = array(
                    'range_from' => $this->input->post('result_from')[$key2],
                    'range_to' => $this->input->post('result_to')[$key2],
                    'range_color' => $this->input->post('result_color')[$key2],
                );
            }
    ...
    public function load_assessment_index($returnflag = 0)
    {
        ...
        if ($rpt_period == "weekly") {
            if ($WeekStartDate != '' && $WeekEndDate != '') {
                ...
                $AccuracySet = $this->assessment_dashboard_model->assessment_index_weekly_monthly($report_by, $WeekStartDate, $WeekEndDate, $report_type, $supervisor_id, $parameter_id = '', $trainer_id, $division_id);
                ...
                $AccuracySet = $this->assessment_dashboard_model->assessment_index_weekly_monthly($report_by, $WeekStartDate, $WeekEndDate, $report_type, $supervisor_id, '', $trainer_id, $division_id);
            ...
            $AccuracySet = $this->assessment_dashboard_model->assessment_index_yearly($report_by, $WeekStartDate, $WeekEndDate, $report_type, $supervisor_id, '', $trainer_id, $division_id);
            for ($i = 1; $i <= 12; $i++) {
                $day = str_pad($i, 2, "0", STR_PAD_LEFT);
    ...
    public function ajax_divisionwise_data()
    {
        $trainer_html = '';
        if ($this->mw_session['company_id'] == "") {
            $company_id = $this->input->post('company_id', TRUE);
        ...
        $SDate = date('Y-m-d', strtotime($start_date));
        $EDate = date('Y-m-d', strtotime($end_date));
        $division_id = ($this->input->post('division_id', TRUE) ? $this->input->post('division_id', TRUE) : 0);
        $trainer_list = $this->common_model->get_selected_values('company_users', 'userid,CONCAT(first_name, " " ,last_name) as fullname', 'status="1" and role ="2" AND company_id="' . $company_id . '" AND division_id="' . $division_id . '" ');
        if (count((array) $trainer_list) > 0) {
            foreach ($trainer_list as $value) {
                $trainer_html .= '<option value="' . $value->userid . '">' . $value->fullname . '</option>';
            }
        }
        $data['trainer_html'] = $trainer_html;
        echo json_encode($data);
    }
