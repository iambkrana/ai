...
class Industry_type extends CI_Controller {
    ...
    public function __construct() {
        ...
        if ($this->session->userdata('awarathon_session') == FALSE) {
            ...
            $acces_management = CheckRights($this->mw_session['user_id'], 'industry_type');
    ...
    public function index() {
        ...
        $this->load->view('industry_type/index', $data);
    }
    public function edit(){
        ...
        if(count((array)$this->input->post()) > 0 ){
            ...
            if (!$data['acces_management']->allow_edit) {
                ...
                $data['result'] = $this->industry_type_model->find_by_id($edit_id);
    ...
    public function DatatableRefresh() {
        ...
        $DTRenderArray = $this->industry_type_model->LoadDataTable($dtWhere, $dtOrder, $dtLimit);
        $output = array(
    ...
    public function submit() {
        ...
        if (!$acces_management->allow_add) {
            $url = base_url().'industry_type';
        }else{
            $this->load->helper('form');
            $this->load->library('form_validation');
            $data['username'] = $this->mw_session['username'];
            $this->form_validation->set_error_delimiters('<div class="alert alert-error"><strong>Error: </strong>', '</div>');
            $this->form_validation->set_rules('description', 'Industry name', 'trim|required|max_length[250]');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->input->post('edit_id')==''){
                    ...
                    $this->common_model->insert('industry_type', $data);
                    $this->session->set_flashdata('flash_message', "Industry Type created successfully.");
                    ...
                    $this->common_model->update('industry_type','id',$edit_id, $data);
                    $this->session->set_flashdata('flash_message', "Industry Type updated successfully.");
    ...
    public function update($role_id){
        ...
        $data['username'] = $this->mw_session['username'];
        $data['rows'] = $this->industry_type_model->fetch_access_data();
        $data['acessdata'] = $this->industry_type_model->find_by_value($role_id);
        $data['result'] = $this->industry_type_model->find_by_id($role_id);
        $this->load->helper('form');
        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters('<div class="alert alert-error"><strong>Error: </strong>', '</div>');
        $this->form_validation->set_rules('description', 'Industry Type name', 'trim|required|max_length[250]');
        $this->form_validation->set_rules('status', 'Status', 'trim|required');
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('industry_type/edit', $data);
        } else {
            $this->industry_type_model->update_role($role_id);
            $this->session->set_flashdata('flash_message', "Industry Type updated successfully.");
            redirect('industry_type');
        }
    }
    public function remove(){
        ...
        if (!$acces_management->allow_delete) {
            ...
            $StatusFlag = $this->industry_type_model->CrosstableValidation(base64_decode($deleted_id));
            if($StatusFlag){
                $this->industry_type_model->remove(base64_decode($deleted_id));  
                $message = "Industry Type deleted successfully.";
            }else{
                $alert_type = 'error';
                $message= "Industry Type cannot be deleted. Reference of Industry Type found in other module!<br/>"; 
            }   
        }
    ...
    public function record_actions($Action) {
        ...
        if ($Action == 1) {
            foreach ($action_id as $id) {
                ...
                $this->common_model->update('industry_type', 'id', $id, $data);
            }
            $message = 'Status changed to active successfully.';
        } else if ($Action == 2) {
            $SuccessFlag=false;
            foreach ($action_id as $id) {
                $StatusFlag = $this->industry_type_model->CrosstableValidation($id);
                if($StatusFlag){
                    $data = array(
                        'status' => 0,
                        'modifieddate' => $now,
                    ...
                    $alert_type = 'error';
                    $message= "Status cannot be change. Reference of Industry Type found in other module!<br/>"; 
                }
            }
            if($SuccessFlag){
            ...
            $SuccessFlag=false;
            foreach ($action_id as $id) {
                $DeleteFlag = $this->industry_type_model->CrosstableValidation($id);
                if($DeleteFlag){
                    $this->common_model->delete('industry_type', 'id', $id);
                    $SuccessFlag=true;
                }else{
                    $alert_type = 'error';
                    $message= "Industry Type cannot be deleted. Reference of Industry Type found in other module!<br/>"; 
            ...
            if($SuccessFlag){
                $message .= 'Industry Type(s) deleted successfully.';
            }
        }
        echo json_encode(array('message' => $message,'alert_type'=>$alert_type));
        exit;
    ...
    public function validate() {
        $status = $this->industry_type_model->validate($this->input->post());
        echo $status;
    }
}
