...
class Ai_cronjob_model_trinity extends CI_Model {
        ...
        if($assessment_type == 3){
            $query = "SELECT 
                ...
                FROM( SELECT DISTINCT 
                    ...
                    CONCAT( du.firstname, ' ', du.lastname ) AS user_name,ar.video_url,ar.vimeo_uri,ar.ftp_status,aa.is_completed,CONCAT('https://player.vimeo.com/video/',ar.vimeo_uri)
    ...
    public function get_schedule(){
        $query  = "SELECT * from trinity_cronjob WHERE process_status=1 order by id ASC LIMIT 0,1";
        $result = $this->db->query($query);
        return $result->result();
    }
    ...
    public function cronjob_status($status){
        $query = "UPDATE trinity_cronjob SET process_started='".$status."'";
        $this->db->query($query);
        $error = $this->db->error();
    ...
    public function update_failed_massage($msg,$company_id,$assessment_id,$user_id,$trans_id,$question_id){
        $now = date('Y-m-d H:i:s');
        $query = "UPDATE ai_schedule SET failed_massage='".$msg."',failed_dttm='".$now."' WHERE 
                company_id='".$company_id."' AND 
                assessment_id='".$assessment_id."' AND 
                user_id='".$user_id."' AND 
                trans_id='".$trans_id."' AND 
    ...
    public function get_your_video($company_id,$assessment_id,$user_id,$trans_id,$question_id){
        ...
        $query  = "SELECT video_url as video_url
                FROM trinity_results WHERE
