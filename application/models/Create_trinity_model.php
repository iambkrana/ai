...
class Create_trinity_model extends CI_Model
{
    ...
    public function LoadDataTable($dtWhere, $dtOrder, $dtLimit)
    {
        ...
        $query = "  SELECT am.id,am.company_id,am.assessment_type as assessment_type_id,
                    ...
                    CASE WHEN am.assessment_type=2 THEN 'Spotlight' WHEN  am.assessment_type=3 THEN 'Trinity' ELSE 'Roleplay' END AS ass_type,
    ...
    public function get_mapped_qna($company_id, $assessment_id, $script_id, $question_id=0){
        $query = "SELECT DISTINCT asq.id, asq.question as question, asq.answer as q_answer, tts.sentence_keyword as answer, tt.is_default FROM assessment_script_qna as asq 
        ...
        if(empty($result) || !empty($result)){ //Condition updated By Anurag - Date:- 07-02-24
            $query = "SELECT id, question, answer, 0 as is_default FROM assessment_script_qna WHERE company_id=$company_id AND script_id=$script_id";
            $query = $this->db->query($query);
            $result = $query->result();
        }
        return $result;
