...
class Ai_process_model extends CI_Model
{
    ...
    //KRISHNA -- TICKET 677 -- Added participant who attempted the Trinity but not completed yet - Need transcript even assessment is not completed
    public function get_process_participants_trinity($company_id, $assessment_id, $start_date, $end_date)
    {
        ...
        FROM(
            SELECT * FROM(
                ...
                SELECT DISTINCT
                    ar.company_id,ar.assessment_id,ar.user_id,tt.id as trans_id,tt.question_id,c.portal_name,am.assessment,
                    CONCAT( du.firstname, ' ', du.lastname ) AS user_name,aq.question,'' as video_url,'' as vimeo_uri,'' as ftp_status,aa.is_completed,aa.attempts, aa.ftpto_vimeo_uploaded
                    ,aa.ftpto_vimeo_dttm as uploaded_dt,'' as process_dt, '' AS datediff, '' as time_diff,aa.addeddate as added_date
                FROM
                    trinity_user_transcripts AS ar
                    LEFT JOIN company AS c ON ar.company_id = c.id
                    LEFT JOIN assessment_mst AS am ON ar.assessment_id = am.id AND ar.company_id = am.company_id
                    LEFT JOIN device_users AS du ON ar.user_id = du.user_id AND ar.company_id = du.company_id 
                    LEFT JOIN trinity_trans as tt ON tt.assessment_id=ar.assessment_id AND tt.question_id=0 
                    LEFT JOIN assessment_script_qna as aq on tt.question_id=aq.id
                    LEFT JOIN assessment_attempts AS aa ON ar.assessment_id = aa.assessment_id AND ar.user_id = aa.user_id 
                WHERE
                    ar.company_id = '$company_id'  ";
                if ($assessment_id != '') {
                ...
                $query .= " AND ar.assessment_id = '$assessment_id'  AND ar.assessment_id = '$assessment_id' AND aa.is_completed =0 
