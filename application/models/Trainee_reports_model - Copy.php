...
class Trainee_reports_model extends CI_Model {
    ...
    public function getPrePostData($workshop_id, $trainee_id = '') {
        $query = " SELECT a.trainee_id,b.workshop_name,IF(a.pre_played_questions=0,'Not Played',CONCAT(a.pre_avg,'%')) AS pre_average,
            IF(a.post_played_questions=0,'Not Played',CONCAT(a.post_avg,'%')) AS post_average,a.post_avg,a.rank,
            a.post_avg-a.pre_avg as ce,CONCAT(du.firstname,' ',du.lastname) as trainee_name
    ...
    public function getLivePrePostData($workshop_id = '', $trainee_id = '') {
        $TodayDt = date('Y-m-d H:i:s');
        $query = "SELECT b.workshop_name,fs.trainee_name, IFNULL(fs.pre_avg,'Not Played') AS pre_average, IFNULL(fs.post_avg,'Not Played') AS post_average, FORMAT(fs.post_avg-fs.pre_avg,2) AS ce
    ...
    public function getPrePostWorkshopwise($workshop_id) {
    ...
    public function getLivePrePostWorkshopwise($workshop_id) {
        ...
        FROM trainee_result AS es WHERE es.workshop_id= $workshop_id AND es.trainee_id IN 
        ...
        AND arp.user_id IN (SELECT distinct user_id FROM atom_results WHERE workshop_id=$workshop_id AND workshop_session='POST')
    ...
    public function get_PrePostTopicwise($workshop_id = '', $trainee_id = '') {
        $TodayDt = date('Y-m-d H:i');
        $query = "SELECT IFNULL(SUM(res.pre_accuracy),0) AS pre_accuracy, IFNULL(SUM(res.post_accuracy),0) AS post_accuracy,res.topic_id, res.subtopic_id, qt.description AS topic,qst.description AS subtopic, IFNULL(FORMAT(SUM(res.pre_accuracy),2),'Not Played') AS pre_average_np, IFNULL(FORMAT(SUM(res.post_accuracy),2),'Not Played') AS post_average_np, IFNULL(SUM(res.post_accuracy)- SUM(res.pre_accuracy),'Not Played') AS ce
        FROM (
    ...
    public function get_Traineewise_Rank($workshop_id = '', $user_id = '') {
        $TasterFlag = true;
        if ($user_id != "") {
        ...
        if (count($LiveSet) > 0) {
            $LcSqlStr = " SELECT z.* FROM (SELECT fs.*,@curRank := @curRank + 1 AS rank FROM(
                ...
                WHERE arp.workshop_session='POST' AND CONCAT(w.pre_end_date,' ', STR_TO_DATE(w.pre_end_time, '%l:%i %p')) >='$TodayDt' 
                AND arp.workshop_id=$workshop_id ";
                if ($TasterFlag) {
    ...
    public function SynchTraineeData($Company_id="",$Workshop_id=""){
        ...
        $query = "SELECT distinct a.workshop_id,b.start_date,b.workshop_type,b.region FROM atom_results as a LEFT JOIN "
                . " workshop as b ON b.id=a.workshop_id where a.company_id= $Company_id "
                . "AND a.workshop_id NOT IN(select workshop_id FROM trainee_result where company_id= $Company_id)";
        if($Workshop_id !=""){
        ...
        if(count($WorkshopSet)>0){
            foreach ($WorkshopSet as $value) {
                ...
                $lcSqlStr = "INSERT INTO trainee_result(company_id,workshop_id,trainee_id,workshop_date,pre_correct,pre_played_questions,
                    pre_total_questions,pre_avg,post_correct,
                    post_played_questions,post_total_questions,post_avg,avg_time,workshop_type,region_id,pre_time_taken,post_time_taken,
                  pre_live_status,post_live_status,total_topic,istester,rank)
                select fs.*, @curRank := @curRank + 1 AS rank FROM(
                SELECT $Company_id AS compnay_id,prpo.workshop_id,prpo.user_id,'".$value->start_date."' AS start_date, SUM(prpo.pre_correct) AS pre_correct, SUM(prpo.pre_played_quesiton) AS pre_played_quesiton, SUM(prpo.pre_total_questions) AS pre_total_questions,
                 FORMAT((SUM(prpo.pre_correct)*100)/ SUM(prpo.pre_total_questions),2) AS pre_average, SUM(prpo.post_correct) AS post_correct,
                  SUM(prpo.post_played_quesiton) AS post_played_quesiton, SUM(prpo.post_total_questions) AS post_total_questions, 
                  FORMAT((SUM(prpo.post_correct)*100)/ SUM(prpo.post_total_questions),2) AS post_average, FORMAT(((SUM(prpo.pre_time_taken)+ SUM(prpo.post_time_taken))/ (SUM(prpo.pre_played_quesiton)+ SUM(prpo.post_played_quesiton))),2) AS avgtime, 
                 '".$value->workshop_type."' AS workshop_type,'".$value->region."' AS region, SUM(prpo.pre_time_taken) AS pre_time_taken, SUM(prpo.post_time_taken) AS post_time_taken, SUM(pre_live) AS pre_live, SUM(post_live) AS post_live,
                count(distinct prpo.topic_id) as total_topic,du.istester
                FROM(
                ...
                GROUP BY wq.company_id,wq.workshop_id,wq.topic_id
                 ...
                 UNION ALL
                SELECT ar.company_id,ar.user_id,ar.workshop_id,0 AS pre_correct,0 AS pre_total_questions,
                 ar.post_correct, IF(powq.liveflag=1, ar.played_quesiton,powq.total_questions) AS post_total_questions, ar.total_seconds,
                 0 AS pre_time_taken,ar.total_seconds_taken AS post_time_taken,
                 0 AS pre_played_quesiton,played_quesiton AS post_played_quesiton,0 AS pre_live, powq.liveflag AS post_live,
                  IF(powq.liveflag=1,ar.topic_id,powq.topic_id) as topic_id
                FROM (
                SELECT arp.company_id,arp.user_id,arp.workshop_id,arp.workshop_session, SUM(arp.is_correct) AS post_correct, SUM(arp.timer) AS total_seconds, 
                SUM(arp.seconds) AS total_seconds_taken, COUNT(arp.question_id) AS played_quesiton,arp.topic_id
                FROM atom_results AS arp
                WHERE arp.workshop_id = $Workshop_id AND arp.workshop_session = 'POST'
                GROUP BY arp.company_id,arp.user_id,arp.workshop_id,arp.topic_id
                ) AS ar
                INNER JOIN (
                SELECT wq.company_id,wq.workshop_id,wq.questionset_id, IF(CONCAT(w.post_end_date,' ', STR_TO_DATE(w.post_end_time, '%l:%i %p')) >= '$CurrentTime', 1, 0) AS liveflag,
                 COUNT(DISTINCT wq.question_id) AS total_questions,wq.topic_id
                FROM workshop_questions AS wq
                INNER JOIN workshop_questionset_post AS wpo ON wq.workshop_id = wpo.workshop_id AND wq.questionset_id = wpo.questionset_id AND
                 wpo.active=1
                INNER JOIN workshop AS w ON w.id=wq.workshop_id
                WHERE wq.workshop_id = $Workshop_id
                GROUP BY wq.company_id,wq.workshop_id,wq.topic_id
    ...
    public function UpdateLiveData($trainee,$Workshop_id=""){
        $query = "SELECT distinct a.workshop_id,a.pre_live_status,a.post_live_status FROM trainee_result as a "
                . "WHERE (a.pre_live_status=1 OR a.post_live_status=1)";
        if($Workshop_id !=""){
            $query .= " AND a.workshop_id=".$Workshop_id;
        }
        $result =$this->db->query($query);
        ...
        if(count($WorkshopSet)>0){
            foreach ($WorkshopSet as $value) {
                $Workshop_id =$value->workshop_id; 
                if($value->pre_live_status){
                    $lcSQlStr ="update trainee_result as a INNER JOIN( SELECT arp.user_id,arp.workshop_id, SUM(arp.is_correct) AS pre_correct,
                        SUM(arp.seconds) AS total_seconds_taken, COUNT(arp.question_id) AS played_quesiton,count(distinct topic_id) as total_topic
                       FROM atom_results AS arp
                       WHERE arp.workshop_id = $Workshop_id AND arp.workshop_session = 'PRE' group by arp.user_id )"
                        . " as b ON b.user_id=b.trainee_id AND b.workshop_id=a.workshop_id"
                        . " SET a.total_topic=b.total_topic,a.pre_correct=b.pre_correct,"
                        . "a.pre_total_questions=b.played_quesiton,a.pre_played_questions=b.played_quesiton,"
                        . "a.pre_avg=(a.pre_correct*100/b.played_quesiton),a.pre_time_taken=b.total_seconds_taken,"
                        . "a.avg_time=(b.total_seconds_taken/played_quesiton)";
                    $this->db->query($lcSQlStr);
                }
                if($value->post_live_status){
                    $query ="select count(distinct topic_id) as total_topic FROM atom_results WHERE arp.workshop_id = $Workshop_id AND "
                            . " workshop_session = 'POST' AND user_id=$trainee "
                            . "AND topic_id NOT IN(select distinct wq.topic_id FROM workshop_questions as wq INNER JOIN "
                            . " workshop_questionset_pre as wkq ON wkq.questionset_id=wq.questionset_id AND wkq.workshop_id=wq.workshop_id"
                            . " WHERE wq.workshop_id=$Workshop_id)";
                    $this->db->query($lcSQlStr);
                    $TopicCountSet= $result->row();
                    $TotalTopic =$TopicCountSet->total_topic;
                    $lcSQlStr ="update trainee_result as a INNER JOIN( SELECT arp.user_id,arp.workshop_id, SUM(arp.is_correct) AS post_correct,
                       SUM(arp.seconds) AS total_seconds_taken, COUNT(arp.question_id) AS played_quesiton
                       FROM atom_results AS arp WHERE arp.workshop_id = $Workshop_id AND "
                        . "arp.workshop_session = 'POST' AND arp.user_id=$trainee)"
                        . " as b ON b.user_id=b.trainee_id AND b.workshop_id=a.workshop_id"
                        . " SET a.total_topic=(a.total_topic+$TotalTopic),a.post_correct=b.post_correct,"
                        . "a.post_total_questions=b.played_quesiton,a.post_played_questions=b.played_quesiton,"
                        . "a.post_avg=(a.post_correct*100/b.played_quesiton),a.post_time_taken=b.total_seconds_taken,"
                        . "a.avg_time=((a.post_time_taken+b.total_seconds_taken)/(a.pre_played_questions+b.played_quesiton)) "
                        . " WHERE a.id=".$value->id;
                    $this->db->query($lcSQlStr);
                }
