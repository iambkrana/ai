...
class Goals_model extends CI_Model {
    ...
    public function LoadDataTable($dtWhere, $dtOrder, $dtLimit) {
        $query = " SELECT p.*,pm.description as parameter_name FROM goal_mst as p ";
        ...
        $query_count = "SELECT COUNT(p.id) as total FROM goal_mst as p ";
        $query_count .= " $dtWhere";
    ...
    public function Check_subparameter_exist($subparameter,$subparameter_id) {        
        $querystr="select description from goal_mst where description='" . $subparameter . "'";
		if($subparameter_id!=''){
