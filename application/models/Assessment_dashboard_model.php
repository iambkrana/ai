...
class Assessment_dashboard_model extends CI_Model
{
    ...
    public function get_Total_Assessment($Company_id, $start_date = '', $end_date = '', $region_id = '', $store_id = '', $report_type, $division_id='', $trainer_id)
    {
        $cond = "";
        $asmg = "";
        ...
        if ($trainer_id != '') {
            $cond .= " AND asm.trainer_id IN  (" . implode(',', $trainer_id) . ") ";
            $asmg = " LEFT JOIN assessment_managers asm ON asm.trainer_id=art.trainer_id ";
        }
        if ($store_id != '') {
            $cond .= " AND du.store_id =" . $store_id;
        }
        if ($division_id != '') {
            $cond .= " AND am.division_id =" . $division_id;
        }
        if ($report_type == 2) {
            $query = "select IFNULL(count(distinct am.id),0) as total_assessment 
                  FROM assessment_trainer_weights art
                    LEFT JOIN assessment_mst am ON am.id=art.assessment_id " .$asmg. " 
    ...
    public function get_Candidate_Assessment($Company_id, $question_type, $report_by, $start_date = '', $end_date = '', $region_id = '', $store_id = '', $report_type, $division_id='', $trainer_id)
    {
        $cond = "";
        $asmg = "";
        if ($start_date != '' && $end_date != '') {
        ...
        if ($trainer_id != '') {
            $cond .= " AND asm.trainer_id IN  (" . implode(',', $trainer_id) . ") ";
            $asmg = " LEFT JOIN assessment_managers as asm ON asm.assessment_id=ar.assessment_id ";
        }
        if ($store_id != '') {
            $cond .= " AND du.store_id =" . $store_id;
        }
        ...
        if ($report_type == 2) {
            $query = "SELECT count(distinct ar.user_id) as assessment_candidate FROM assessment_final_results ar
                    LEFT JOIN assessment_mst am ON am.id = ar.assessment_id " .$asmg. "  
    ...
    public function get_Candidate_Assessment_old($Company_id, $question_type, $report_by, $start_date = '', $end_date = '', $region_id = '', $store_id = '', $report_type)
    {
    ...
    public function get_Average_Accuracy($Company_id, $report_by, $start_date = '', $end_date = '', $region_id = '', $store_id = '', $report_type, $assessment_id1, $division_id='', $trainer_id)
    {
        $cond = "";
        $assmng = "";
        if ($start_date != '' && $end_date != '') {
        ...
        if ($division_id != '') {
            $cond .= " AND b.division_id!='' AND b.division_id !='0' AND b.division_id =" . $division_id;
        }
    ...
    public function get_top_five_parameter($Company_id, $report_by, $SDate = '', $EDate = '', $region_id = '', $store_id = '', $report_type, $assessment_id1, $division_id='', $trainer_id='')
    {
        $cond = "";
        $assmng = "";
        if ($SDate != '' && $EDate != '') {
        ...
        if ($division_id != '') {
            $cond .= "   AND b.division_id =" . $division_id;
        }
        if ($trainer_id != '') {
            $cond .= " AND asm.trainer_id IN  (" . implode(',', $trainer_id) . ") ";
            $assmng = " LEFT JOIN assessment_managers asm ON asm.assessment_id=a.assessment_id ";
        ...
        if ($report_type == "2") {
            if ($report_by == 0) {
                ...
                LEFT JOIN assessment_mst as b ON b.id=a.assessment_id ".$assmng." 
                ...
                LEFT JOIN assessment_mst as b ON b.id=a.assessment_id
                LEFT JOIN parameter_mst pm ON pm.id=a.parameter_id ".$assmng." 
                LEFT JOIN device_users du ON du.user_id=a.user_id WHERE 1=1";
            }
            $query .= $cond;
            if ($report_by == 0) {
    ...
    public function get_bottom_five_parameter($Company_id, $report_by, $top_five_para_id, $SDate = '', $EDate = '', $region_id = '', $store_id = '', $report_type, $assessment_id1, $division_id='', $trainer_id)
    {
        $cond = "";
        $assmng = "";
        if ($SDate != '' && $EDate != '') {
    ...
    public function overall_result_parameter($isoverall, $passrange_from, $passrange_to, $failrange_from, $failrange_to, $SDate = '', $EDate = '', $region_id = '', $assessment_string = '', $step = 0, $store_id = '', $report_type, $division_id='', $trainer_id)
    {
        $lcwhere = "";
        if ($SDate != '' && $EDate != '') {
        ...
        if ($division_id != '') {
            $lcwhere .= "AND am.division_id != '' AND am.division_id != '0' AND am.division_id = " . $division_id;
        }
        if ($trainer_id != '') {
            $mang = "  LEFT JOIN assessment_managers as asm on asm.assessment_id = am.id ";
            $lcwhere .= " AND asm.trainer_id IN  (" . implode(',', $trainer_id) . ") ";
        }
        if ($report_type == "2") {
            ...
            $query1 .= " FROM (
                select du.region_id,art.parameter_id,count(atm.user_id) as total_users
                        ...
                        INNER JOIN device_users du ON du.user_id=atm.user_id  " . $mang . " 
            ...
            $query1 .= " union all 
                        ...
                        LEFT JOIN assessment_mst am ON am.id=art.assessment_id
                        INNER JOIN device_users du ON du.user_id=art.user_id " . $mang . "
                        WHERE 1=1 $lcwhere
    ...
    public function overall_result_assessment($isoverall, $passrange_from, $passrange_to, $failrange_from, $failrange_to, $SDate = '', $EDate = '', $region_id = '', $assessment_string = '', $step = 0, $store_id = '', $report_type, $division_id, $trainer_id)
    {
        $lcwhere = "";
        $mang = '';
        if ($SDate != '' && $EDate != '') {
        ...
        if ($trainer_id != '') {
            $mang = " Inner Join assessment_managers as asm on asm.assessment_id = am.id";
            $lcwhere .= " AND asm.trainer_id IN  (" . implode(',', $trainer_id) . ") ";
        }
        if ($report_type == "2") {
    ...
    public function get_region_average($Company_id, $report_by, $region_id = '', $SDate = '', $EDate = '', $assessment_id = '', $report_type, $supervisor_id, $division_id='', $trainer_id)
    {
        $cond = "";
        $cond1 = "";
        $cond2 = '';
        ...
        if ($trainer_id != '') {
            $assmng = " LEFT JOIN assessment_managers as amg on amg.assessment_id=art.assessment_id ";
            $cond .= " AND amg.trainer_id IN (" . implode(',', $trainer_id) . ")";
        }
        if ($report_by == 1) {
    ...
    public function get_region_result($Company_id, $report_by, $region_id = '', $SDate = '', $EDate = '', $assessment_id = '', $report_type, $supervisor_id, $division_id, $trainer_id)
    {
        $cond = '';
        $cond1 = '';
        $assmng = "";
    ...
    public function get_parameter_assessment_score($Company_id, $parassess_id = '', $region_id = '', $StartDate = '', $EndDate = '', $report_type, $store_id = '', $division_id='', $trainer_id)
    {
        $query = " SELECT CONCAT(IFNULL(FORMAT(avg(art.accuracy),2),0),'%') AS result,du.region_id,
                    art.user_id,pm.id,art.assessment_id,am.assessment,pm.description as parameter
                    FROM assessment_trainer_weights art
        ...
        if ($division_id != '') {
            $query .= 'AND asm.division_id =' . $division_id;
        }
        if ($trainer_id != '') {
            $query .= " AND asm.trainer_id IN (" . implode(',', $trainer_id) . ")";
        }
        $query .= " group by art.assessment_id ";
        ...
        if ($report_type == "2") {
            $query = " SELECT  IF(am.ratingstyle=2, CONCAT(IFNULL(FORMAT(SUM(art.percentage)/ COUNT(pm.id),2),0),'%'), CONCAT(IFNULL(FORMAT(SUM(art.score)*100/ SUM(pm.weight_value),2),0),'%')) AS result,du.region_id,
                    ...
                    FROM assessment_results ar 				 	   
                        ...
                        LEFT JOIN assessment_mst am ON am.id=ar.assessment_id	
                        LEFT JOIN assessment_managers asm ON am.id=asm.assessment_id 
                        INNER JOIN device_users du ON du.user_id=ar.user_id
            ...
            $query = " SELECT  IF(am.ratingstyle=2, CONCAT(IFNULL(FORMAT(SUM(ar.score)/ COUNT(pm.id),2),0),'%'), CONCAT(IFNULL(FORMAT(AVG(ar.score),2),0),'%')) AS result,du.region_id,
                    IF(am.ratingstyle=2,IFNULL(SUM(ar.score),0),IFNULL(SUM(ar.score),0)) AS rating,
                    IF(am.ratingstyle=2,IFNULL(count(pm.id),0),IFNULL(SUM(pm.weight_value),0))as total_rate,
                    pm.id,ar.assessment_id,ar.company_id,
                    am.assessment,pm.description as parameter,aq.question,am.is_weights
                    FROM ai_subparameter_score ar
                    ...
                    $dtWhere AND parameter_type='parameter' AND ar.question_id !=''";
            $query .= " group by ar.question_id ";
            $query_count = $query;
            $query .= " $dtOrder $dtLimit ";
            $result = $this->db->query($query);
            $data['ResultSet'] = $result->result_array();
    ...
    public function get_trainer($Company_id, $report_by = '', $StartDate, $EndDate, $division_id='')
    {
        $query = "SELECT  distinct ar.id as assessment_id, ar.assessment as assessment FROM assessment_mst ar 	
			LEFT JOIN assessment_managers am ON am.assessment_id=ar.id where 1=1 ";
        if ($StartDate != '' && $EndDate != '') {
    ...
    {
        $query = "SELECT DISTINCT d.id, d.division_name, CONCAT(c.first_name,' ',c.last_name) as trainer_name 
              from assessment_managers as asp 
              LEFT JOIN division_mst as d on d.id = asp.division_id 
              LEFT JOIN company_users as c on c.division_id = d.id where 1=1 AND c.role='4' ";
        if ($division_id != '') {
    ...
    public function assessment_index_weekly_monthly($report_by, $StartDate = '', $EndDate = '', $report_type, $supervisor_id, $user_id = '', $trainer_id, $division_id='')
    {
        $cond = "";
        $ResultArray = array();
        $PeriodArray = array();
        ...
        if ($division_id != '') {
            $cond .= " AND amg.division_id =" . $division_id;
        }
        if ($report_type == "2") {
    ...
    public function assessment_index_yearly($report_by, $StartDate = '', $EndDate = '', $report_type, $supervisor_id, $user_id = '', $trainer_id, $division_id)
    {
        $cond = '';
        $cond = '';
        $assmng = "";
        ...
        if ($report_type == "2") {
            $query = " SELECT FORMAT(avg(art.accuracy),2) AS result,month(am.start_dttm) as wmonth,DATE_FORMAT(am.start_dttm,'%d') wday
                    FROM assessment_final_results art  LEFT JOIN assessment_mst am ON am.id=art.assessment_id ".$assmng."          
                    WHERE 1=1 ";
            $query .= $cond;
            ...
            $query = " SELECT FORMAT(avg(art.score),2) AS result,month(am.start_dttm) as wmonth,DATE_FORMAT(am.start_dttm,'%d') wday
                        FROM ai_subparameter_score art  LEFT JOIN assessment_mst am ON am.id=art.assessment_id ".$assmng."
                        WHERE parameter_type='parameter' AND 1=1 ";
            $query .= $cond;
            $query .= " group by month(am.start_dttm) ";
        } else {
