...
class Adoption extends MY_Controller
{
    ...
    //     if (count((array)$assessment_list) > 0) {
    //         foreach ($assessment_list as $value) {
    //             $assessment_html .= '<option  value="' . $value->user_id . '">' . $value->department . '</option>';
    //         }
    //     }
    ...
    //             $assessment_count = count($LAssessmentDetails);
    //             for ($i = 0; $i < $assessment_count; $i++) {
    ...
    //                 $SDate = date('Y-m-d', strtotime("-29 days"));
    //                 $EDate = date("Y-m-d");
    //                 $getUserstart = $this->adoption_model->GetUserAssessmentwise($lastAssessmentId, $Company_id, $trainer_id, $Region_id, $Division_id, $SDate, $EDate);
    //             } elseif ($IsCustom == 'Current Year') {
    //                 $SDate = date('Y-01-01');
    //                 $EDate = date("Y-m-d");
    //                 $getUserstart = $this->adoption_model->GetUserAssessmentwise($lastAssessmentId, $Company_id, $trainer_id, $Region_id, $Division_id, $SDate, $EDate);
    ...
    //         }
    //     } else {
    //         if ($IsCustom == 'Current Year') {
    //             $SDate = date('Y-01-01');
    //             $EDate = date("Y-m-d");
    //             $getUserstart = $this->adoption_model->GetUserAssessmentwise($Assessment_id, $Company_id, $trainer_id, $Region_id, $Division_id, $SDate, $EDate);
    //         } else {
    //             $getUserstart = $this->adoption_model->GetUserAssessmentwise($Assessment_id, $Company_id, $trainer_id, $Region_id, $Division_id, $SDate, $EDate);
    //         }
    ...
    public function adoption_by_divison_overall($returnflag = 0)
    {
        ...
        if ($Assessment_id == '' && $DivisionSet == '') {
            ...
            if (isset($LAssessmentDetails)) {
                $Assessment_id = isset($LAssessmentDetails[0]['id']) ? $LAssessmentDetails[0]['id'] : '';
        ...
        $Rdata['complete_count'] = json_encode($completed, JSON_NUMERIC_CHECK);
        $Rdata['mapped_count'] = json_encode($user_mapped, JSON_NUMERIC_CHECK);
        $ad_byDivision = $this->load->view('adoption/AdoptionByDivisionOverall', $Rdata, true);
    ...
    public function adoption_by_manager($returnflag = 0)
    {
        ...
        $a_b_managers = $this->load->view('adoption/adoption_by_manager', $Rdata, true);
    ...
    public function ad_by_region($returnflag = 0)
    {
        ...
        if ($Assessment_id == '' and $Region_id == '' and $Manager_id == '') {
            if ($IsCustom == "") {
                $CurrentDate = date("Y-m-d");
                $LAssessmentDetails = $this->adoption_model->last_expierd_assessment($CurrentDate, $Company_id);
            } else if ($IsCustom == "Current Year") {
                $startdate = date('Y-01-01');
                $CurrentDate = date("Y-m-d");
        ...
        $ab_region = $this->load->view('adoption/adoption_by_region', $Rdata, true);
