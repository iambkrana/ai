                            ...
                            <div class="panel-group accordion" id="accordion3">
                                ...
                                <div class="portlet light bordered">
                                    ...
                                    <div class="portlet-body">
                                        <form id="frmArea" name="frmArea" method="post">
                                            ...
                                            <form id="frmModalForm" name="frmModalForm" onsubmit="return false;"> 
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="row">    
                                                        <div class="col-md-9">       
                                                            <div class="form-group">
                                                                <label class="">Area Name<span class="required"> * </span></label>
                                                                <input type="text" name="area_name" id="area_name" maxlength="250" class="form-control input-sm" autocomplete="off">
...
<script src="<?php echo $base_url; ?>assets/global/plugins/datatables/Buttons-1.3.1/js/dataTables.buttons.min.js" type="text/javascript"></script>
        <script>
            ...
            jQuery(document).ready(function() {
                ...
                jQuery.validator.addMethod("areaDuplicateCheck", function(value, element){                   
                var isSuccess = false;   
                $.ajax({
                    type: "POST",
                    data: {area_name:value,id:area_edit_id,company_id:$('#company_id').val()},
                    url: "<?php echo base_url(); ?>area/Check_Area",
                    async: false,
...
//                    },
                    ...
                    submitHandler: function (form) {
                        ...
                        form_success.show();
                        form_error.hide();
                        $.post('<?php echo base_url();?>index.php/area/submit', $("#frmModalForm").serialize(), function (data) {
                            //console.log(data);
                    ...
                    messages: {
                        area_name: {
                            ...
                            remote: "This same Area name already exists. Please try another Name."
                        },
            ...
            function LoadCreateModal(){
                $('.modal-title').html('Create Area').show();
                $('#frmModalForm').trigger("reset");
                $("#frmModalForm").validate().resetForm();
                $("#status").select2("val", "1");                
            ...
            function LoadEditModal(edit_id){
                ...
                $('.modal-title').html('Edit Area').show();
                $('#frmModalForm').trigger("reset");
                $("#frmModalForm").validate().resetForm();
                $("#status").select2("val", "1");
                $('#edit_id').val(edit_id);
                $.ajax({
                    ...
                    url: "<?php echo $base_url; ?>area/edit",
                    success: function (response_json) {
                        var response= JSON.parse(response_json);
                        //console.log(response);
                        if (response.message==''){
                             area_edit_id = response.result[0].id;
                             $("#company_id").select2("trigger", "select", { 
                                data: { id: response.result[0].company_id,text:response.result[0].company_name} 
                            });
                            $('#area_name').val(response.result[0].area_name);
                    ...
                    buttons: {
                        ...
                        action: function(){
                            $.ajax({
                                ...
                                url: "<?php echo $base_url; ?>area/record_actions/"+opt,
                                success: function (response_json) {
                                    var response= JSON.parse(response_json);
                                    ShowAlret(response.message,response.alert_type);
                                    DatatableRefresh();
                ...
                if (Check > 0){
                    if( opt == 1 ){
                       LoadConfirmDialog("Confirm Active Status ?",opt);  
                        ...
                        LoadConfirmDialog("Delete Selected Area(s) ?",opt);
                    }   
                }else{
            ...
            function LoadDeleteDialog(Division,Id){
                $.confirm({
                    title: 'Confirm!',
                    content: " Are you sure you want to delete Area '"+Division+"' ? ",
                    buttons: {
                        confirm:{
                        text: 'Confirm',
                        btnClass: 'btn-orange',
                        ...
                        action: function(){
                            $.ajax({
                                type: "POST",
                                url: "<?php echo $base_url;?>area/remove",
                                data: {deleteid:Id},
                                success: function (response_json) {
                                    var response= JSON.parse(response_json);
                                    ShowAlret(response.message,response.alert_type);
            ...
            {
                var x = 0;
                for (var i = 0; i < frmArea.elements.length; i++)
                {
                    if (frmArea.elements[i].checked == true)
                    {
                        x++;
                    }
                }
...
//                }
                ...
                table.dataTable({
                    ...
                    "processing": true,
                    "serverSide": true,
                    "sAjaxSource": "<?php echo base_url() . 'area/DatatableRefresh'; ?>",
            ...
            $('#company_id').select2({
                    ...
                    separator: ',',
                    ajax: {
                        url: "<?php echo base_url();?>area/ajax_company",
                        dataType: 'json',
                        quietMillis: 100,
                    ...
                    initSelection: function(element, callback) {
                        return $.getJSON("<?php echo base_url();?>area/ajax_company?id=" + (element.val()), null, function(data) {
