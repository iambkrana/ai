AssessmentForm.validate({
  errorElement: "span",
  errorClass: "help-block help-block-error", //create_trinity_my.js
  focusInvalid: false,
  ignore: "",
  rules: {
    assessment_type: {
...
jQuery.validator.addMethod(
  ...
  function (value, element) {
    ...
    $.ajax({
      ...
      url: Base_url + "create_trinity_my/Check_assessment",
      async: false,
  ...
  "Assessment already exists!!!33"
);
jQuery.validator.addMethod(
  "alphanumeric",
  function (value, element) {
      return this.optional(element) || /^[\p{L}\p{N}0-9-( )#_]+$/i.test(value);
  ...
  $.confirm({
    ...
    buttons: {
      confirm: {
        ...
        action: function () {
          $("#Row-" + r).remove();
          Selected_QuestionArray.splice(
            $.inArray(r, Selected_QuestionArray),
            1
          );
...
function UpdateAssessment() {
  ...
  $.ajax({
    ...
    async: true,
    type: "POST",
    url: Base_url + "create_trinity_my/update/" + Encode_id,
    dataType: "json",
    data: form_data,
      ...
      if (Data["success"]) {
        ...
        setTimeout(function () {
          ...
          window.location.href =
            Base_url + "create_trinity_my/edit/" + Encode_id + "/1.2";
        }, 500);
      } else {
        $("#errordiv").show();
    ...
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      ShowAlret(
        "Status: " + textStatus + " ,Contact Mediaworks for technical support!"
      );
    },
  });
}
  ...
  $.confirm({
    ...
    buttons: {
      confirm: {
        ...
        action: function () {
          $.ajax({
            url: Base_url + "create_trinity_my/RemoveParticipantUser/" + Encode_id,
            type: "POST",
            data: {
              Remove_id: Remove_id,
            },
      ...
      confirm: {
        ...
        action: function () {
          $.ajax({
            ...
            url:
              Base_url + "create_trinity/Removeall_participants/" + Encode_id,
      ...
      confirm: {
        ...
        action: function () {
          $.ajax({
            ...
            data: fdata,
            url:
              Base_url + "create_trinity/Removeall_supermapping/" + Encode_id,
            success: function (response_json) {
              var response = JSON.parse(response_json);
              ShowAlret(response.message, response.alert_type);
...
function LoadFilterUserData(is_mapped) {
  $("#UserFilterTable").dataTable({
    ...
    sAjaxSource:
      Base_url +
      "create_trinity/UsersFilterTable/" +
      Encode_id +
      "/" +
      is_mapped,
    fnServerData: function (sSource, aoData, fnCallback) {
...
function LoadFilterSupervisorData() {
  $("#SupervisorFilterTable").dataTable({
    ...
    serverSide: true,
    sAjaxSource:
      Base_url + "create_trinity/SupervisorsFilterTable/" + Encode_id,
    fnServerData: function (sSource, aoData, fnCallback) {
      aoData.push({
        name: "__mode",
        value: "featuredimage.ajaxload",
    ...
    if (txn_id !== "") {
      $.each(TempSubParameterArray, function (key, value) {
        if (txn_id == value.txn_id) {
          weight = isNaN(value.parameter_weight)
            ? 0
            : parseInt(value.parameter_weight);
          total_weight += weight;
        }
      });
    }
...
function check_overall_parameter_weights(TempSubParameterArray, TotalQue) {
  ...
  if (TempSubParameterArray.length > 0) {
    ...
    var sum_weight = 0;
    $.each(TempSubParameterArray, function (key, value) {
      param_weight = isNaN(value.parameter_weight)
        ? 0
        : parseInt(value.parameter_weight);
  ...
  if (Object.keys(TempSubParameterArray).length > 0) {
    ...
    TempSubParameterArray.forEach(function (item, index) {
      if (parseInt(txn_id) == parseInt(item.txn_id)) {
        ...
        if (
          x > 0 &&
          parseInt(temp_parameter_id) != parseInt(item.parameter_id)
        ) {
        ...
        if (parseInt(temp_parameter_id) != parseInt(item.parameter_id)) {
          ...
          if (
            Object.keys(subparameters_counter).length >= 0 &&
            typeof subparameters_counter[0].length != "undefined"
          ) {
          ...
          div_html =
            div_html +
            '<div class="ps-container"><div class="parameter-badge">' +
            item.parameter_label_name +
            "</div>";
        }
        if (y == 0) {
          div_html =
            div_html +
            '<div class="ps-seperator">&nbsp;</div><div class="sub-parameter-badge">' +
            item.subparameter_name;
          temp_weight = item.parameter_weight
        ...
        if (item.type_name != "") {
          html_sentence_keyword =
            ...
            item.type_name +
            ':</span><p style="font-weight:normal">' +
            item.sentence_keyword +
            "</p>";
        } else {
          html_sentence_keyword =
            html_sentence_keyword +
        ...
        if (parseInt(y) == parseInt(total_subparameter - 1)) {
          div_html =
            div_html +
            '</div><div class="ps-seperator">&nbsp;</div><div class="parameter-weight-badge">' +
            temp_weight;
          div_html =
            div_html +
            '</div><div class="ps-seperator">&nbsp;</div><div class="keysent-badge">' +
            html_sentence_keyword +
            "</div>";
          html_sentence_keyword = "";
        }
        if (
          parseInt(temp_parameter_id) !== parseInt(item.parameter_id) ||
          Object.keys(temp_json).length == x
        ) {
          div_html = div_html + "</div>";
        }
...
function UploadXlsTrainee() {
  ...
  $.ajax({
    ...
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      ShowAlret(
        "Status: " + textStatus + " ,Contact Atomapp for technical support!"
      );
    },
  });
}
...
function SaveAssessment(mode) {
  ...
  // }
  if (mode == "C") {
    var url = Base_url + "create_trinity_my/submit/" + Encode_id;
  } else {
    url = Base_url + "create_trinity_my/submit";
  }
      ...
      if (Data["success"]) {
        ...
        if (AddEdit == "A" || AddEdit == "C") {
          setTimeout(function () {
            // wait for 5 secs(2)
            window.location.href =
              Base_url + "create_trinity_my/edit/" + Data["id"] + "/1.2";
          }, 500);
        } else {
          questionTable();
        }
      ...
      confirm: {
        ...
        action: function () {
          $.ajax({
            ...
            url:
              Base_url +
              "create_trinity/send_notification/" +
              action +
              "/" +
              Encode_id,
            beforeSend: function () {
...
function showEmptyImg() {
  $("#EmptyImage").show();
  $("#Emptytext").show();
  $("#EmptyVoice").show(); //DARSHIL ADDED for trinity voice - 26.02.24
  ...
  $.ajax({
    ...
    success: function (Odata) {
      ...
      if (Oresult) {
        ...
        $("#queImage").show();
        $("#Emptytext").hide();
        $("#EmptyVoice").hide(); //DARSHIL ADDED for trinity voice - 26.02.24
        $("#EmptyImage").hide();
      }
      customunBlockUI();
    },
...
//DARSHIL ADDED - 26.02.24
//PERSONA VOICE POPUP DISPLAY
//START
$("#persona_voice_option").change(function () {
  var voice_select = $("#persona_voice_option").val();
  if (voice_select !== "") {
    if (voice_select == "male") {
      $.confirm({
        title: "Are you sure you want to select male voice?",
        content: "",
        buttons: {
          confirm: {
            text: "Confirm",
      ...
      $.confirm({
        title: "Are you sure you want to select female voice?",
        content: "",
        buttons: {
          confirm: {
            text: "Confirm",
  ...
  $.ajax({
    ...
    success: function (Odata) {
      ...
      if (Data["Success"] && Data["Message"] != "") {
        ...
        $("#EmptyImg").prop(
          "src",
          Base_url + "assets/uploads/trinity/" + Data["que_image"]
        );
      } else {
...
function map_persona() {
  ...
  //DARSHIL ADDED THIS VARIABLE - 26.02.24
  //BY DEFAULT WE ARE SHOWING THIS IMAGE WHEN THE USER HAS NOT SELECTED ANY IMAGE.
  //SO WHILE CREATING A PERSONA IF WE DO NOT SELECT ANY IMAGE AND UPLOAD IT
  //THEN THE "NO IMAGE" IMAGE IS STORED IN THE DATABASE.
  var no_image_persona = Base_url + "assets/uploads/no_image.png";
  ...
  if (persona_id == 0 && persona_id != "") {
    ...
    var persona_voice = $("#persona_voice_option").val(); //DARSHIL ADDED THIS FOR TRINITY VOICE - 26.02.24
  ...
  //DARSHIL UPDATED THIS CONDITIONS - 26.02.24

  if (
    persona_id == "" &&
    persona_image == no_image_persona &&
    persona_voice == ""
  ) {
    ShowAlret(
      "Please Select Persona Name, Persona image & Persona Voice..!!",
      "error"
    );
    return false;
  }
  if (persona_id == "") {
    ShowAlret("Please Select Persona Name..!!", "error");
    return false;
  }
  if (persona_image == "" || persona_image == no_image_persona) {
    //if persona image is not selected and still the user tries to save the "no-image" image by the system then this error message will be shown
    ShowAlret("Please Select Persona image..!!", "error");
    return false;
  }
  if (persona_voice == "") {
    ShowAlret("Please Select Persona Voice..!!", "error");
    return false;
  }

  formData.append("persona_id", persona_id);
  formData.append("file", persona_image);
  formData.append("persona_voice", persona_voice); //DARSHIL ADDED THIS FOR TRINITY VOICE - 26.02.24
  $.ajax({
    ...
    success: function (Odata) {
      ...
      if (Data["Success"] && Data["Message"] != "") {
        ...
        setTimeout(function () {
          // wait for 5 secs(2)
          window.location.href =
            Base_url + "create_trinity/edit/" + Encode_id + "/1.3";
        }, 500);
      } else {
        ShowAlret(Data["Message"], "error");
        $("#errordiv").show();
    ...
    $.ajax({
      ...
      success: function (Odata) {
        ...
        if (Data["lchtml"] != "") {
          ...
          // $('#LoadModalFilter').data('modal', null);
          Selected_QuestionArray =
            Selected_QuestionArray.concat(NewQuestionArray);
          NewQuestionArray = [];
          Totalqstn = Data["tr_no"];
          $(".language_id").on("select2:select", function (e) {
            var data = e.params.data;
    ...
    success: function (Odata) {
      if (Odata["success"] == 1) {
        ...
        setTimeout(function () {
          // wait for 5 secs(2)
          window.location.href =
            Base_url + "create_trinity/edit/" + Odata["assessment_id"] + "/2";
        }, 500);
      } else {
  ...
  if (txn_id != "" && parameter_id != "") {
    ...
    $("input[name='chksp_list[]']").each(function (index) {
      ...
      if ($("#chksp_id" + temp_id).prop("checked")) {
        TempSubParameterArray = TempSubParameterArray.filter(function (obj) {
          if (
            parseInt(obj.txn_id) == parseInt(txn_id) &&
            parseInt(obj.parameter_id) == parseInt(parameter_id) &&
            parseInt(obj.subparameter_id) == parseInt(temp_id)
          ) {
            return false;
          } else {
            return true;
        ...
        if (
          $("#type_id" + temp_id).css("display") == "none" ||
          $("#type_id" + temp_id).css("visibility") == "hidden"
        ) {
        } else {
          type_id = $("#type_id" + temp_id).val();
          type_name = $("#type_id" + temp_id + " option:selected").text();
          ...
          if (type_id !== "" && sentence_keyword === "") {
            ShowAlret(
              "Please add Sentence/Keyword for " + parameter_name,
              "error"
            ); //mandatory textarea value in case of selected sentence/keyword
            is_valid = 1;
            return false;
          }
          sentence_keyword = sentence_keyword.replace(/\n/g, " ");
    ...
    var TempPSMCount = TempPSMArray.filter(function (objs) {
      if (
        parseInt(objs.txn_id) == parseInt(txn_id) &&
        parseInt(objs.parameter_id) == parseInt(parameter_id)
      ) {
        return true;
      } else {
        return false;
    ...
    success: function (Odata) {
      ...
      if (Data["Success"] && Data["Message"] != "") {
        ...
        setTimeout(function () {
          // wait for 5 secs(2)
          window.location.href =
            Base_url + "create_trinity/edit/" + Data["Encode_id"] + "/2";
        }, 500);
        $("#errordiv").hide();
        $("#errorlog").html("");
      } else {
