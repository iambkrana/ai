...
jQuery.validator.addMethod("numericcheck", function (value, element) {
    ...
    $.ajax({
        ...
        url: Base_url + "assessment_create/numeric_check",
        async: false,
        success: function (msg) {
            abc = msg != "" ? false : true;
        }
...
//     //return this.optional(element) || /[^\p{L}\p{N}\( )#_]+/ug.test(value);  // add by shital Language Module : 06-02-2024
// }, 
// "Only letters, numbers, round brackets and underscores please");
...
function UpdateAssessment() {
    ...
    var lanaguge_id = $('.language_id').val();
    form_data['set_language_id'] = lanaguge_id;
...
function ConfirmUsers(flag = '') {
    ...
    $.ajax({
        ...
        data: {
            ...
            division_id: $('#flt_division_id').val(),
            flag: flag
...
function DivisionManagerSupervisor() {
    if($('#division_id').val()=='' || $('#division_id').val()==null){
        // ShowAlret("Please select Division.", 'error');
        return false;
    }
    $.ajax({
        url: Base_url + "assessment_create/MappedManagerSupervisor",
        type: 'POST',
        data: {
...
function LoadFilterUserData(is_mapped) {
    $('#UserFilterTable').dataTable({
        ...
        "fnServerData": function (sSource, aoData, fnCallback) {
            ...
            aoData.push({ name: 'flt_division_id', value: $('#flt_division_id').val() });
            aoData.push({ name: 'flt_division', value: $('#flt_division_id :selected').text() });
...
function add_managers() {   
    ...
    $.ajax({
        ...
        url: Base_url + "assessment_create/addManagers/"+divisionId,
        success: function (response_json) {
            $('#modal-body_ms').html(response_json);
            $('#LoadModalFilter_ms').modal();
            // add by shital LM : 06-02-2024
            $('.select2, .select2-multiple').select2().on('select2:open', function (e) {
                $('.select2-container').addClass('notranslate');
                $('.select2, .select2me').addClass('notranslate');
...
function add_supervisor() {    
    ...
    $.ajax({
        type: "POST",
        data: {division_id: $('#division_id').val()},
        url: Base_url + "assessment_create/addSupervisors/"+divisionId,
        success: function (response_json) {
            $('#modal-body_ms').html(response_json);
            $('#LoadModalFilter_ms').modal();
            // add by shital LM : 06-02-2024
...
$('#pdf_report_title').text("PDF Report Language Access:");
